...
export default function DashboardClient() {
  ...
  return (
    <div className="flex flex-col h-full w-full">
      {/* 検索ヘッダー (デスクトップ) */}
      <header className="hidden sm:flex bg-white/70 backdrop-blur-xl border-b border-slate-200 px-6 py-4 items-center justify-between sticky top-0 z-40">
      ...
      {/* モバイル検索 */}
      <div className="sm:hidden px-4 pt-16 pb-2 bg-white/50 backdrop-blur-md sticky top-0 z-30">
      ...
      <main className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 sm:space-y-6 bg-[#f8fafc] custom-scrollbar">
        ...
        {sortedApps.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 bg-white rounded-2xl border border-dashed border-slate-200">
            ...
            <Button variant="link" className="mt-3 text-indigo-600 font-bold text-sm" onClick={() => { setSearch(''); setStatusFilter('全て'); window.history.pushState(null, '', '/') }}>
              リセットする
            </Button>
          </div>
        )}
